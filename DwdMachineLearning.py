...
def prep_data_for_ml(k_factor_g=3, parameter_n=1):
...
def build_and_compile_model(shape=5, parameter=1):
    ...
    if parameter == 2:
        ...
        combo = layers.concatenate([x_3, x_2, x], name="ConnectHiddenLayers")
        y = layers.Dense(32, activation="relu", name="HiddenLayerForConnection")(combo)
        outputs = layers.Dense(1, name="OutputsDataForMyLocation")(y)

        model = keras.Model(inputs=[inputs, input_2, input_3], outputs=outputs, name="dnn_model")
...
def machine_learning_modelling(shape=5, parameter=1):
    counter_new_model = 0
    for i in range(0,30,1):
        if parameter == 1:
            my_df, index_for_plot, column_names, data_density, dist, height, my_bool, index_for_plot_2, column_name_list_2, data_density_2, dist_2, height_2, my_bool_2 = prep_data_for_ml(k_factor_g=shape, parameter_n=parameter)
            if my_bool == False:
                pass
            else:
                print(column_names)
                ...
                if parameter == 2:
                    ...
                    history = dnn_model.fit(
                        ...
                        train_labels,
                        validation_split=0.2, batch_size=1200,
                    ...
                    history = dnn_model.fit(
                        [train_features, dist_1, height_1],
                        train_labels,
                        validation_split=0.2, batch_size=1,
                        verbose=0, epochs=1)
                    print(dnn_model.evaluate([test_features, dist_2, height_2], test_labels, verbose=0))
        ...
        if parameter == 1:
            ...
            if my_bool == False:
                ...
                if parameter == 2:
                    ...
                    print(dnn_model.evaluate([dataset, dist_2, height_2], real_data, verbose=0))
...
# machine_learning_modelling(shape=5, parameter=1)
